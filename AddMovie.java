...
public class AddMovie implements HttpHandler {
    ...
    public void handlePut(HttpExchange r) throws IOException, JSONException {
    		try {
    			...
    			result = session.run("MATCH (n:movie {id:" + "'" +id+ "'" + "})" + "\n" + "RETURN n.name");
    			result2 = session.run("MATCH (n:movie {id:"+ "'" +id+ "'" + "})" + "\n" + "RETURN n.id");
    				
    			if(result.list().isEmpty() && result2.list().isEmpty()) {
    				session.run("Create (:movie {name:\"" +name+ "\", id:" + "'" + id+ "'" + "})");
    			...
    			if(!(node_name.equals(name)) && str_id.equals(id)) {
    				...
    				session.run("MATCH (n:movie {name:\"" +node_name+ "\", id:" + "'" + str_id+ "'" + "})" + "\n" + "SET n.name =" + "'" + name + "'"+ "\n" + "RETURN n.name");	
    				r.sendResponseHeaders(200, -1);
    				return;
    			} 
